...
configured_targets reth-act-pull-request lint.yml > "$tmp_dir/lint-targets"
assert_unique "$tmp_dir/lint-targets" lint
if ! diff -u "$tmp_dir/lint-targets" <(configured_targets reth-act-merge-queue lint.yml); then
  echo "PR and merge-queue lint targets differ" >&2
  exit 1
fi
...
configured_targets reth-act-pull-request unit.yml > "$tmp_dir/unit-targets"
assert_unique "$tmp_dir/unit-targets" unit
if ! diff -u "$tmp_dir/unit-targets" <(configured_targets reth-act-merge-queue unit.yml); then
  echo "PR and merge-queue unit targets differ" >&2
  exit 1
fi
